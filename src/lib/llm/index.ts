...
import { DEFAULT_POLICY } from "../policy-core";
...
硬性约束：
1. 总估时不得超过剩余可投入时间（策略容量约 {CAP_TOTAL} 分钟）；放不下时必须合并任务或砍掉低优先级任务，并在 reason 中说明放弃了什么。
...
export async function replanGoal(input: ReplanInput): Promise<ReplanResult> {
  const client = getLlmClient();
  // Phase 12：提示词容量来自策略声明（无声明时回退唯一默认值 policy-core）
  const capTotal =
    input.declaredMinutesPerDay?.reduce((s, v) => s + v, 0) ??
    input.daysLeft * DEFAULT_POLICY.dailyCapacityMinutes;
  const system = REPLANNER_SYSTEM.replace("{CAP_TOTAL}", String(capTotal));
  const text = await client.complete(system, JSON.stringify(input));
