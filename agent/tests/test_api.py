...
class TestHealth:
    def test_shape(self, api_client):
        ...
        assert body["graph"] == "analyze->[github_tool]->[calendar_tool]->progress_analysis->plan|replan->validate->finalize"
        assert body["tools"] == ["github(readonly)"]  # CAL_ICS_PATH 未配置时 calendar 停用
