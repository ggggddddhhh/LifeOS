"""LangGraph 装配：Analyze → Plan/Replan → Validate → Finalize。
唯一回边是 Validate→Plan/Replan 的重试边，由 attempts<MAX_ATTEMPTS 门控，
attempts 每次经过 Validate 单调 +1 ⇒ 结构上不存在死循环（LLM 调用 ≤ 2 次）。"""
...
def build_graph(llm: LLM):
    ...
    g.add_conditional_edges("analyze", route_kind, ["plan", "replan"])
...
def run_plan(request: dict, llm: LLM) -> AgentState:
    graph = build_graph(llm)
    return graph.invoke({"kind": "plan", "request": request, "attempts": 0, "llm_calls": 0})


def run_replan(request: dict, llm: LLM) -> AgentState:
    graph = build_graph(llm)
